...
class AudioLibrary(QMainWindow):
    ...
    def build_ui(self):
        ...
        self.open_location_btn.clicked.connect(self.open_location)
        self.open_location_btn.setEnabled(False)
        file_actions.addWidget(self.open_location_btn)
        ...
        file_actions.addWidget(self.copy_path_btn)

        right_layout.addLayout(file_actions)
    ...
    def scan_finished(self, count):
        ...
        self.collapsed_folders = {
            str(Path(s["relative"]).parent) if str(Path(s["relative"]).parent) != "." else "Root"
        ...
        for sound in self.sounds:
            ...
            if folder != current_folder:
                ...
                heading = QListWidgetItem(
                    f"{'▶' if folder in self.collapsed_folders else '▼'}  📁  {folder}"
                )
                heading.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
                heading.setData(Qt.UserRole, None)
                heading.setData(Qt.UserRole + 1, folder)
                heading.setToolTip("Click to collapse/expand this folder")
                self.list.addItem(heading)

            if folder in self.collapsed_folders:
                continue
    ...
    def list_item_clicked(self, item):
        ...
        # Folder heading: toggle collapse/expand.
        if not path:
    ...
    def selection_changed(self, row):
        ...
        # Folder heading — clear file actions.
        if not path:
            self.current_path = None
            self.name_label.setText("Select a sound")
            self.path_label.setText("")
            self.open_location_btn.setEnabled(False)
            self.copy_path_btn.setEnabled(False)
            return
    ...
    def collapse_all(self):
        # Collapse every currently visible folder.
        folders = set()
        for sound in self.sounds:
            relative = Path(sound["relative"])
            folder = str(relative.parent)
    ...
    def copy_path(self):
        ...
        QApplication.clipboard().setText(str(Path(self.current_path).resolve()))
        self.status.setText("Copied full file path to clipboard.")
